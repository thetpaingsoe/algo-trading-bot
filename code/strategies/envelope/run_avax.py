...
binance = BinanceFutures(api_setup, is_demo)
...
trigger_orders = binance.fetch_open_trigger_orders(params['symbol'])
long_orders_left = 0
short_orders_left = 0
for order in trigger_orders:
    if order['side'] == 'buy' and order['info']['tradeSide'] == 'open':
        ...
        short_orders_left += 1
    binance.cancel_trigger_order(order['id'], params['symbol'])
...
if open_position:
    ...
    # exit
    binance.place_trigger_market_order(
    ...
    # sl
    sl_order = binance.place_trigger_market_order(
        symbol=params['symbol'],
        side=close_side,
        amount=amount,
...
if long_ok:
    for i in range_longs:
        ...
        if amount >= min_amount:
            # entry           
            binance.place_trigger_limit_order(
                symbol=params['symbol'],
                side='buy',
                amount=amount,
