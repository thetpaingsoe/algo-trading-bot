...
class BinanceFutures():
    def __init__(self, api_setup: Optional[Dict[str, Any]] = None, is_demo: bool = False) -> None:
        
        if api_setup == None:
            self.session = ccxt.binance()
        else:
        ...
        if is_demo:
            ...
            self.session.verbose = True  # debug output
    ...
    def fetch_min_amount_tradable(self, symbol: str) -> float:
        try:
            ...
            raise Exception(f"Failed to fetch minimum amount tradable: {self.markets}")        
    ...
    def fetch_open_positions(self, symbol: str) -> List[Dict[str, Any]]:
        try:
            positions = self.session.fetch_positions([symbol], params={'productType': 'future', 'marginCoin': 'USDT'})
            real_positions = []
